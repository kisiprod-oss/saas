...
| **Contrat de bail** | Bail imprimable au format A4, rempli à partir du contrat saisi : parties, objet, durée, loyer, dépôt de garantie, charges, clause résolutoire. Le document rappelle qu'il n'est pas rédigé par un juriste. |
| **Papier à en-tête** | Quittances et baux portent le logo de l'agence en filigrane, son cachet circulaire et ses mentions légales. Ornements dissuasifs : ils rendent l'imitation pénible, ils ne la rendent pas impossible. |
| **Vérification en ligne** | Chaque quittance et chaque bail reçoit un code tiré au hasard, imprimé en clair et dans un QR code. N'importe qui — locataire, propriétaire, tribunal — le vérifie sans compte sur `/verifier/<code>` : agence, numéro, date, montant. C'est cela qui protège d'une contrefaçon, pas le filigrane. Le nom du locataire y est abrégé, la page étant publique. |
| **Registre des documents** | *Documents émis* liste chaque quittance et chaque bail édité : destinataire, code de vérification, qui l'a sorti, quand, et combien de fois. Une ligne par document, pas par impression. Le registre note l'édition, il ne stocke pas de fichier : les documents se réimpriment à l'identique depuis leur fiche. |
| **Remise au locataire** | Une quittance part par e-mail (un lien vers le document) et son code de réception part par WhatsApp. Le locataire saisit le code sur la page du document : l'agence sait qu'il l'a reçu, et le locataire n'a plus à le prouver. La remise en main propre se note aussi. |
| **Ce que l'accusé prouve** | Qu'à une date donnée, quelqu'un détenant le code l'a saisi. C'est un commencement de preuve entre gens de bonne foi, **pas une signature électronique** : il n'identifie pas formellement la personne. Une confirmation faite depuis l'espace locataire, après authentification, est distinguée et conservée comme telle. |
...
| **Acompte sur arriérés** | Un locataire qui doit plusieurs mois et verse une somme unique : l'agence saisit le montant reçu, le logiciel propose une répartition en commençant par le mois le plus ancien, et **l'agence corrige chaque ligne à la main** si le locataire a demandé autre chose. Un règlement est écrit par facture concernée. Le serveur revérifie les arriérés réels au moment de valider : impossible d'imputer sur une facture plus que ce qu'elle doit, même si l'écran a été laissé ouvert. |
...
| **Page « Courte durée »** | Un onglet public destiné aux **particuliers** qui louent un meublé à la nuitée (le public type des plateformes de voyageurs). Il présente les fonctions courte durée, dit franchement ce que Sen Gestion **n'est pas** (il n'apporte pas de voyageurs), et propose d'être rappelé. Chaque demande crée un **prospect de l'éditeur** — pas une demande d'agence — visible dans l'administration avec un lien WhatsApp et un suivi (nouveau / rappelé / client / perdu). Les photos de la page se changent depuis l'écran d'administration ; sans photo, des illustrations neutres s'affichent. |
...
| **Formules** | Page tarifs publique, limites appliquées automatiquement selon l'abonnement. La formule Découverte reste gratuite sans limite de durée. |
| **Limite du gratuit** | La formule gratuite émet 5 factures par mois. Au-delà, l'agence gère toujours ses biens, ses locataires et ses loyers, et ses factures déjà émises restent imprimables : seule l'émission de nouvelles factures attend le mois suivant ou un abonnement. Le compte gratuit se compte par boîte aux lettres — les alias `+` et les points Gmail n'en rouvrent pas un second. |
...
| **Aide à la saisie** | Sur la création d'une facture ou d'un bail, l'agence décrit ce qu'elle veut en une phrase — « le loyer de mars pour Awa Ndiaye, plus 15 000 de régularisation d'eau » — et le formulaire se pré-remplit. **L'IA ne crée jamais le document** : elle prépare les champs, affiche ce qu'elle a compris et ce qui manque, et c'est l'agence qui relit et valide. Elle ne choisit que parmi les biens, locataires et baux réels de l'agence, et chaque identifiant renvoyé est revérifié côté serveur. Sans clé `ANTHROPIC_API_KEY`, l'encadré ne s'affiche pas et les formulaires fonctionnent comme avant. |
| **Comptabilité** | Un tableau de bord sépare ce que l'agence a encaissé, ses honoraires (son vrai chiffre d'affaires) et ce qui reste dû aux propriétaires — trois chiffres bien distincts, jamais confondus. Choix de la période (mois, trimestre, année), deux graphiques (facturé/encaissé, honoraires) sur douze mois glissants, détail par bien, et une version imprimable en PDF à en-tête de l'agence. Ce n'est pas une comptabilité légale : ni plan comptable, ni journal, elle sert à se contrôler et à remettre des chiffres nets à un expert-comptable. |
| **Abonnements encaissés** | L'agence règle son abonnement en ligne (Orange Money, Wave, Free Money, carte) sur le compte marchand de **l'éditeur** — à ne pas confondre avec « Encaissement », où chaque agence branche *son* compte pour recevoir *ses* loyers. La formule et son échéance ne sont accordées qu'après vérification du règlement **auprès de l'opérateur** : une fausse notification n'accorde rien. Un renouvellement anticipé prolonge la période en cours au lieu de l'écraser. **Verrou : tant que `EDITEUR.statut` n'est pas `"societe"`, le module refuse d'encaisser** — les CGU publiées promettent la gratuité, et facturer sans NINEA ni RCCM exposerait l'éditeur. |
| **Abonnements par carte (Stripe)** | S'**ajoute** à PayDunya sans le remplacer : une agence qui ne paie qu'en Orange Money ou Wave garde son moyen de paiement. Stripe n'accepte pas les entreprises sénégalaises et ne facture pas en FCFA — ces règlements passent donc par une entité de l'éditeur dans un pays couvert, **en euros**, convertis au taux **fixe** du franc CFA (655,957 XOF = 1 €), jamais à un cours flottant. En base, `montant` reste toujours en FCFA pour que les deux fournisseurs s'additionnent ; `montant_devise` garde ce qui a réellement été débité. Signature du webhook vérifiée **et** session relue auprès de Stripe avant d'accorder quoi que ce soit. Même verrou d'immatriculation que PayDunya. |
| **Vue d'ensemble (éditeur)** | Réservée aux adresses `ADMIN_EMAILS` : nombre d'agences inscrites, payantes, **actives ou dormantes** (une agence sans facture depuis 30 jours est celle qu'il faut rappeler), courbe des inscriptions sur douze mois, détail par agence, et la liste de tous les comptes ouverts. Le montant des abonnements y est affiché comme **théorique** et l'écran le dit : rien n'enregistre encore qu'une agence a réellement payé. |
...
| **Devis** | Un particulier demande un devis à un professionnel (candidature validée uniquement) directement depuis l'annuaire, sans créer de compte. L'artisan répond avec un prix depuis son espace ; le particulier accepte ou refuse via un lien personnel, puis l'artisan marque le projet terminé — ce qui ouvre automatiquement le droit à un avis. **Sen Gestion ne touche jamais l'argent du projet** : le paiement se règle directement entre les deux parties. Formule gratuite : 3 devis répondus par mois, puis un abonnement (formule « Devis Pro ») pour continuer. |
